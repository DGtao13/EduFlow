...
class PrivateLessonsViewModel(private val database: EduFlowDatabase) : ViewModel() {
    ...
    fun delete(lesson: RecurringPrivateLesson) = viewModelScope.launch { database.recurringPrivateLessonDao().delete(lesson) }
}

class PrivateLessonEditorViewModel(private val database: EduFlowDatabase, id: Long?) : ViewModel() {
...
class PrivateLessonEditorFactory(private val database: EduFlowDatabase, private val id: Long?) : ViewModelProvider.Factory {
    @Suppress("UNCHECKED_CAST") override fun <T : ViewModel> create(modelClass: Class<T>): T = PrivateLessonEditorViewModel(database, id) as T
}
