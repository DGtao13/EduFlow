...
class BackupRepository(private val context: Context, private val database: EduFlowDatabase = EduFlowDatabase.getInstance(context)) {
    ...
    private suspend fun insert(d: BackupData, personalOnly: Boolean = false) {
        ...
        d.privateLessons.forEach { dto ->
            ...
            database.recurringPrivateLessonDao().upsert(RecurringPrivateLesson(dto.id,dto.subjectId,dto.weekdays?.firstOrNull() ?: dto.weekday,LocalTime.parse(dto.start),LocalTime.parse(dto.end),LocalDate.parse(dto.startDate),dto.endDate?.let(LocalDate::parse),dto.interval,dto.teacher,dto.room,privateName,dto.enabled,locationKind))
            database.recurringPrivateLessonWeekdayDao().deleteForSource(dto.id)
            database.recurringPrivateLessonWeekdayDao().insertAll((dto.weekdays ?: listOf(dto.weekday)).distinct().sorted().map { RecurringPrivateLessonWeekday(dto.id, it) })
        }
        ...
        if (!personalOnly) d.slots.forEach { database.scheduleSlotDao().upsert(ScheduleSlot(it.id,it.templateId,it.weekday,it.lessonIndex,LocalTime.parse(it.start),LocalTime.parse(it.end),it.subjectId,it.teacher,it.room,it.group,it.logicalBlockId)) }
        if (!personalOnly) SchoolBlockNormalization.normalizeLegacySlots(database)
